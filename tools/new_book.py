...
new_book.py -- register a new source and scaffold its dossier directory.
    ...
    python3 tools/new_book.py --id B5 \
        --title "Pre-Suasion" \
        --author "Some Author" --year 2020 \
...
What it does, and only this:
  ...
  2. creates sources/<ID>/                   (empty, ready for dossiers)
  3. regenerates registry/compiled/*

It edits NOTHING that already exists. That is the whole point: Book 5 cannot
damage Book 1 because Book 5 only ever adds files. See docs/INGESTION.md for
...
def main() -> int:
    ...
    if not re.fullmatch(r"B\d+", args.id):
        print("error: --id must look like B5", file=sys.stderr)
        return 2
