...
\item \textbf{Limits} -- when the move fails, what it costs the user, and the
...
\item \textbf{See also:} -- other entry numbers. The book is a lattice, not a
  line; follow the links when an entry assumes one.
\item \texttt{T-\,nn-\,nn} -- entry number: chapter, then position in chapter.
\end{itemize}
\par\endgroup
