...
Produce a **mapping table** before writing anything. `registry/ingest/B4-map.md`
is a complete worked example — B4 (Cialdini) was ingested against an existing
tree of 27 entries written from B1 and B3, so it exercises every row of the
table above. Abridged:

```
ch. 2  reciprocation            -> T-04-02 (exists, deepens)   dossier + one sentence
                                  T-19-01 (new, c19)           The Uninvited Debt
                                  T-24-01 (new, c24)           Refusing an Unauthorised Debt
ch. 4  social proof             -> T-15-01 (new, c15)          The Crowd as Evidence
                                  T-13-01 (new, c13)           Manufactured Consensus
ch. 6  authority                -> T-08-01                     WITHDRAWN -- see below
                                  T-18-01 (new, c18)           The Symbols of Authority
```

Two things in that map are worth copying as practice:

* **It records revisions.** The ch. 6 row for `T-08-01` was planned as an
  additive dossier and withdrawn on contact with the entry, because the
  authority material would have deepened nothing there and a dossier backing no
  sentence is padding. The withdrawal is written down with the reason rather
  than silently applied, so the next reader does not re-litigate it. A map that
  quietly disagrees with the tree is worse than no map.
* **It explains the register weight.** B4 was given weight 85, below B1's 90,
  and the map says why that does not decide *empirical* claims — weights are
  tie-break priorities for strategic questions, and on mechanism B4 is the only
  laboratory source, so there is no tie to break.

Commit the mapping table as `registry/ingest/<ID>-map.md`. It is the plan of
...
```bash
python3 tools/new_dossier.py --book B5 --topic T-18-01 \
        --locator "ch.2, pp.47-63"
```

(In a reviewed batch, `hb_author.dossier()` does the same thing and refuses to
overwrite — see AGENTS.md.)
...
* `unique` — anything this source says that no other source says. **Set
  ...
  will refuse to let that book be dropped from the entry's `@sources`, forever.
...
Commit in small batches: `sources(B5): T-18-01, T-15-03`.
...
Use the helper. Do not hand-edit the source list:

```python
from hb_author import add_source
add_source("T-04-02", "B4",
           mechanism_addition="One sentence that only this book supports.")
```

It appends the sentence to `mechanism` and adds the book to **both** the header
`@sources` and the footer `\sources{}` line, which must match (validated).

The helper reads the entry's current source list rather than assuming register
order. Entries do not list their books in id order — `T-04-03` is `B3, B1` — so
a hand-rolled replacement built on `B1, B3` matches nothing, changes nothing,
and reports success. The build then fails R10 because a dossier exists for a
book the entry never cited, which reads like a rule bug rather than a typo in
the merge. This happened; the helper is the fix.

If you do edit by hand, the smallest change that carries the material is:
...
**File no dossier at all** if the new book supports nothing the entry lacks. A
dossier that backs no sentence is padding, and R7 bans padding — record the
withdrawn mapping in the ingest map instead, with the reason.
