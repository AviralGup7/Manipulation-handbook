...
Two conditions make it reliable. The change must arrive after the decision and not before it, so that it presents as an unfortunate discovery rather than as the actual offer. And it must be small enough to fall inside the range the person can absorb without reopening the whole question --- which is a range set by how much they have already invested, and grows with it. B1's street layer: never pay ahead of delivery beyond a small deposit --- money in advance removes every compulsion to perform --- and never okay work before a price is agreed, since \emph{do whatever it takes} is an open cheque written on your side of the table.
\end{mechanism}
