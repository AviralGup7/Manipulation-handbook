...
The important property is that both respond to the \emph{form} of the move rather than to the intent behind it. Knowing the sequence is scripted does not switch either off, which is why the technique survives being recognised. What it purchases is not agreement with a position but a felt duty to meet a halfway point that was never halfway. B1 scopes the move: most arguments are sleeping dogs --- the mature test is whether a vital interest is engaged --- so the deepest concession is declining fights that were never yours. B3's version: concede by demonstration rather than argument, because deeds persuade without creating a point to attack.
